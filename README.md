...
| 대시보드 UI | "Recharts 월별 라인차트, 카테고리 바차트, Scope 도넛차트, KPI 카드" | 차트 컴포넌트 구현 | 실무자/경영자 구분 없이 직관적 설계 방향 |
...
| 배포 트러블슈팅 | "Netlify Functions에서 Prisma 500 에러 디버깅" | 원인 분석 + 수정 | 환경변수 미설정 → pg TCP 어댑터 전환 최종 판단 |
